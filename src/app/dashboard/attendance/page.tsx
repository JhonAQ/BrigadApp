...
import { 
  Camera, QrCode, CheckCircle2, XCircle, RefreshCw, 
  Settings, FileText, Download, Clock 
} from 'lucide-react';
...
export default function AttendancePage() {
  const [activeTab, setActiveTab] = useState<'scanner' | 'config' | 'reports'>('scanner');
  ...
  const fetchSettings = async () => {
    const { data } = await supabase.from('settings').select('*').eq('key', 'attendance_limit_time').single();
  ...
  const exportExcel = () => {
      ...
      const dataToExport = reports.map(r => ({
          ...
          DNI: r.users?.dni || 'N/A',
          Fecha: r.date,
          Hora: new Date(r.created_at).toLocaleTimeString('en-US', { hour12: true, hour: '2-digit', minute: '2-digit' }),
      ...
      <div className="flex-1 bg-white rounded-3xl overflow-hidden shadow-xl border border-slate-100">
            ...
            <div className="w-full h-full bg-black relative flex flex-col items-center justify-center">
                ...
                {!isScanning && scannedUser && (
                    <div className="absolute top-4 right-4 z-40">
                         <Button onClick={resetScan} size="sm" variant="secondary" className="bg-white/90 hover:bg-white text-slate-900 border-none shadow-lg">                                   
                ...
                <AnimatePresence mode="wait">
                    {isScanning ? (
                        ...
                        >
                            {hasPermission ? (
                                ...
                                <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 bg-slate-900 p-8 text-center">                                            
                                    ...
                                    <p className="font-bold text-white mb-2">Sin acceso a la cámara</p>                                                                                                       
                                    <p className="text-sm">Por favor permite el acceso a la cámara para escanear.</p>                                                                              
                                    <Button variant="ghost" className="text-indigo-400 mt-4" onClick={() => window.location.reload()}>
                                        Recargar página
                                    </Button>
                                </div>
                            )}
                        ...
                        >
                            ...
                            <h3 className="text-xl font-bold text-slate-800 mb-1">{scannedUser.name}</h3>                                                                                   
                            <p className="text-xs font-bold text-indigo-600 mb-6 bg-indigo-50 inline-block px-3 py-1 rounded-full uppercase tracking-wide">{scannedUser.role}</p>                                             
                            
                            <div className="grid grid-cols-2 gap-3 mb-6">
                                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col items-center">                                                                                                 
                                    <p className="text-[10px] text-slate-400 uppercase font-bold mb-1 tracking-wider">Hora Escaneada</p>
                                    <p className="font-bold text-slate-700 text-lg">{new Date().toLocaleTimeString('en-US', {hour: '2-digit', minute:'2-digit', hour12: true})}</p>                               
                                </div>
                                <div className={`p-4 rounded-xl border flex flex-col items-center justify-center ${scannedUser.isLate ? 'bg-red-50 border-red-100' : 'bg-emerald-50 border-emerald-100'}`}>                                                                                                
                                    <p className={`text-[10px] uppercase font-bold mb-1 tracking-wider ${scannedUser.isLate ? 'text-red-400' : 'text-emerald-500'}`}>Estado</p>
                                    <p className={`font-bold text-lg ${scannedUser.isLate ? 'text-red-600' : 'text-emerald-600'}`}>
                            ...
                            <div className="space-y-3 pt-4 border-t border-slate-100"> 
                                <p className="text-xs text-slate-400 font-bold uppercase text-left pl-1 mb-2">Evaluación de Uniforme</p>                                                       
                                <div className="grid grid-cols-2 gap-3">
                                    <Button onClick={() => registerAttendance(true)} className="bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-500/20 py-6">                                                
                                        <CheckCircle2 className="w-5 h-5 mr-2" />       
                                        Completo
                                    </Button>
                                    <Button onClick={() => registerAttendance(false)} variant="destructive" className="bg-red-500 hover:bg-red-600 shadow-lg shadow-red-500/20 py-6">                                                                                    
        ...
        {activeTab === 'config' && (
            <div className="p-8 max-w-2xl mx-auto w-full">
                <div className="mb-8">
                    <h2 className="text-2xl font-bold text-slate-800">Reglas de Asistencia</h2>
                    <p className="text-slate-500 mt-1">Configura el horario de tolerancia para marcar a un usuario como tardanza.</p>
                </div>
                ...
                <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                    ...
                    <div className="flex gap-4">
                        ...
                        />
                        <Button onClick={handleSaveConfig} disabled={isSavingConfig} className="bg-indigo-600 hover:bg-indigo-700 h-auto py-3 px-6 rounded-xl shadow-md">
                            {isSavingConfig ? 'Guardando...' : 'Guardar Cambios'}
                        </Button>
                    </div>
                    <p className="text-xs text-slate-400 mt-3 flex flex-col gap-1">
                        <span>• Los escaneos antes o exactamente a esta hora son <b>Puntuales</b>.</span>
                        <span>• Los escaneos un minuto después serán marcados como <b>Tardanza</b>.</span>
                    </p>
                </div>
            </div>
        )}
            ...
            <div className="p-6 h-full flex flex-col">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p className="text-sm text-slate-500">Visualiza y descarga la asistencia.</p>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        />
                        <Button onClick={exportExcel} disabled={loadingReports || reports.length === 0} className="bg-emerald-600 hover:bg-emerald-700 text-white">
                            ...
                            {loadingReports ? (
                                <tr>
                                    <td colSpan={5} className="text-center py-8 text-slate-400">
                                        <RefreshCw className="w-6 h-6 animate-spin mx-auto mb-2 opacity-50" /> Caragando datos...
                                    </td>
                                </tr>
                            ) : reports.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="text-center py-8 text-slate-400">No se encontraron registros para este mes.</td>
                                </tr>
                            ) : (
                                reports.map((row) => (
                                    <tr key={row.id} className="bg-white border-b border-slate-100 hover:bg-slate-50/50">
                                        ...
                                        <td className="px-6 py-4 text-slate-600 font-medium">
                                            {row.date} <span className="text-slate-400 ml-1">
                                                {new Date(row.created_at).toLocaleTimeString('en-US', { hour12: true, hour: '2-digit', minute: '2-digit' })}
                                            </span>
                                        </td>
