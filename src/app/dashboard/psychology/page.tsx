...
export default function PsychologyPage() {
  const [activeTab, setActiveTab] = useState<'PENDIENTE' | 'ATENDIDA'>('PENDIENTE');
           ...
           <div className="flex-1 overflow-y-auto p-2 space-y-2">
             {incidents.map((incident) => (
                ...
                >
                  ...
                  <h3 className="font-semibold text-slate-800 truncate">{incident.student?.firstName} {incident.student?.lastName}</h3>
          ...
          {selectedIncident ? (
            <>
               <div className="p-6 border-b border-slate-100 flex justify-between items-start bg-slate-50/50">
                 <div>
                   ...
                   <p className="text-sm text-slate-500">
                     {selectedIncident.student?.grade}° {selectedIncident.student?.section} • DNI: {selectedIncident.student?.dni}
                   </p>
                 </div>
               ...
               <div className="p-6 flex-1 overflow-y-auto">
                 <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 mb-6">
                   ...
                   <div className="mt-3 text-xs text-amber-700/60 font-medium">
                     Reportado por: {MOCK_USERS.find(u => u.id === selectedIncident.reporterId)?.name} • {selectedIncident.date}
                   </div>
                 </div>
               ...
               <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
                 <Button variant="ghost" onClick={() => setSelectedIncident(null)}>Cancelar</Button>
                 {activeTab === 'PENDIENTE' && (
                    <Button variant="secondary" onClick={handleMarkAsAttended} className="text-rose-600 hover:text-rose-700 bg-rose-50 hover:bg-rose-100 border-rose-200 border">
