...
import { 
  ChevronRight, ChevronLeft, Maximize, Minimize, Presentation, 
  TrendingUp, AlertTriangle, Users, BrainCircuit, Calendar, 
  Award, Target, ShieldCheck, FileSpreadsheet
} from 'lucide-react';
import { 
...
export default function ReportsPresentationArgsPage() {
  ...
  const COLORS = ['#4f46e5', '#ec4899', '#06b6d4', '#14b8a6', '#f59e0b', '#ef4444'];
  ...
  const fetchReportData = async () => {
    ...
    try {
      ...
      const { data: currentIncidents } = await supabase
        .from('incidents')
        .select('*, students(first_name, last_name, grade, section), users(name, role)')
  ...
  const processData = (current: any[], prev: any[]) => {
    ...
    const growth = totalPrev === 0 ? 100 : Math.round(((totalCurrent - totalPrev) / totalPrev) * 100);
    
    const psychologyCases = current.filter(i => i.needs_psychology).length;
    const resolvedCases = current.filter(i => i.status === 'ATENDIDA' || i.status === 'RESUELTO').length;
    ...
    const typeChartData = Object.keys(typeCount).map(k => ({ name: k, value: typeCount[k] }));
    ...
    const gradeChartData = Object.keys(gradeCount).map(k => ({ name: k, Casos: gradeCount[k] })).sort((a,b) => b.Casos - a.Casos);
    ...
    const studentCount = current.reduce((acc: any, inc) => {
      ...
      if (!acc[studentId]) {
        const studentName = inc.students ? `${inc.students.first_name} ${inc.students.last_name}` : 'Desconocido';
        acc[studentId] = { name: studentName, grade: `${inc.students?.grade} ${inc.students?.section}`, count: 0 };
      }
    ...
    setStats({
      ...
      growth,
      psychologyCases,
      resolutionRate: totalCurrent > 0 ? Math.round((resolvedCases / totalCurrent) * 100) : 0,
      typeChartData,
      gradeChartData,
      topStudents,
      topBrigadiers
  ...
  const SLIDES = [
    ...
    () => (
      <div className="flex flex-col items-center justify-center h-full text-center space-y-8 animate-in fade-in zoom-in duration-700">
        ...
        <p className="text-slate-500 font-medium max-w-xl mx-auto mt-4 text-lg">
          Sesión de revisión ejecutiva con el cuerpo de Brigadieres y Coordinadores.
        </p>
      </div>
    ),
    ...
    () => (
      <div className="flex flex-col h-full p-8 md:p-12">
        <h2 className="text-3xl font-black text-slate-800 mb-10 flex items-center gap-3 border-b pb-4">
          <Target className="w-8 h-8 text-indigo-600" /> Indicadores Clave de Rendimiento (KPIs)
        </h2>
        ...
        <div className="grid grid-cols-2 gap-8 h-full">
          <div className="bg-gradient-to-br from-indigo-500 to-indigo-700 rounded-3xl p-8 text-white shadow-xl shadow-indigo-500/20 flex flex-col justify-between relative overflow-hidden">
            ...
            <div>
              <p className="text-indigo-100 font-bold uppercase tracking-wider text-sm mb-2">Total Casos Registrados</p>
            ...
            <div className="bg-white/20 backdrop-blur-md self-start px-4 py-2 rounded-xl mt-4">
              <p className="font-semibold text-sm flex items-center gap-2">
                {stats.growth > 0 ? '📈' : '📉'} {Math.abs(stats.growth)}% {stats.growth > 0 ? 'más' : 'menos'} que el mes anterior
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-rows-2 gap-8">
            <div className="bg-white border text-slate-800 border-slate-200 rounded-3xl p-8 shadow-sm flex items-center justify-between">
              <div>
                <p className="text-slate-500 font-bold uppercase tracking-wider text-xs mb-1">Tasa de Resolución</p>
                <h3 className="text-4xl font-black text-emerald-600">{stats.resolutionRate}%</h3>
                <p className="text-sm font-medium text-slate-400 mt-2">Casos marcados como "Resueltos"</p>
              </div>
            ...
            <div className="bg-white border text-slate-800 border-slate-200 rounded-3xl p-8 shadow-sm flex items-center justify-between">
              <div>
                <p className="text-slate-500 font-bold uppercase tracking-wider text-xs mb-1">Derivaciones a Psicología</p>
                <h3 className="text-4xl font-black text-rose-600">{stats.psychologyCases}</h3>
                <p className="text-sm font-medium text-slate-400 mt-2">Requieren atención profesional</p>
              </div>
    ...
    () => (
      <div className="flex flex-col h-full p-8 md:p-12">
        <h2 className="text-3xl font-black text-slate-800 mb-8 flex items-center gap-3 border-b pb-4">
          <AlertTriangle className="w-8 h-8 text-amber-500" /> Análisis de Infracciones
        </h2>
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-8 min-h-0">
          <div className="bg-white border border-slate-100 rounded-3xl shadow-sm p-6 flex flex-col items-center">
            <h3 className="text-lg font-bold text-slate-700 mb-4">Distribución por Tipo</h3>
            <div className="w-full flex-1 min-h-0">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip formatter={(value: number) => [`${value} casos`, 'Frecuencia']} />
          ...
          <div className="bg-white border border-slate-100 rounded-3xl shadow-sm p-6 flex flex-col">
            <h3 className="text-lg font-bold text-slate-700 mb-4">Afectación por Grados</h3>
            <div className="w-full flex-1 min-h-0">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={stats.gradeChartData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" horizontal={false} />
                  <XAxis type="number" />
                  <YAxis dataKey="name" type="category" width={100} tick={{fontSize: 12, fontWeight: 'bold'}} />
                  <Tooltip cursor={{fill: '#f1f5f9'}} />
                  <Bar dataKey="Casos" fill="#6366f1" radius={[0, 4, 4, 0]} barSize={32} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
    ...
    () => (
      <div className="flex flex-col h-full p-8 md:p-12">
        <h2 className="text-3xl font-black text-slate-800 mb-8 flex items-center gap-3 border-b pb-4">
          <Users className="w-8 h-8 text-rose-500" /> Foco de Atención: Estudiantes Recurrentes
        </h2>
        
        <div className="bg-white border border-rose-100 rounded-3xl shadow-xl shadow-rose-50/50 p-8 flex-1 overflow-hidden">
          <p className="text-slate-500 mb-6 font-medium">Top 5 estudiantes con mayor cantidad de incidencias registradas este mes (Requieren Intervención):</p>
          
          <div className="space-y-4">
            {stats.topStudents.length > 0 ? stats.topStudents.map((student: any, i: number) => (
              <div key={i} className="flex items-center justify-between p-5 bg-slate-50 hover:bg-rose-50 transition-colors border-l-4 border-rose-500 rounded-r-xl">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-lg font-bold text-slate-800 uppercase">{student.name}</h4>
                    <p className="text-sm text-slate-500 font-medium">Grado/Sección: {student.grade}</p>
                  </div>
                </div>
                <div className="text-right">
                  <span className="text-3xl font-black text-rose-600">{student.count}</span>
                  <span className="text-sm font-bold text-rose-400 block -mt-1">Incidencias</span>
                </div>
              </div>
            )) : (
              <div className="text-center py-20 text-slate-400 font-bold">No hay estudiantes recurrentes este mes. ¡Excelente entorno!</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col h-full p-8 md:p-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 flex-1">
          <div className="md:col-span-1 bg-amber-50 rounded-3xl p-8 border border-amber-200 flex flex-col justify-center items-center text-center">
            ...
            <h3 className="text-2xl font-black text-amber-900 mb-2">Brigadieres de Alto Impacto</h3>
            <p className="text-amber-700/80 font-medium">Este ranking reconoce a los brigadieres con mayor nivel de reporte proactivo y aseguramiento del orden mensual.</p>
          </div>
          
          <div className="md:col-span-2 bg-white rounded-3xl p-8 border border-slate-200 shadow-lg">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.topBrigadiers} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} />
                <XAxis dataKey="name" tick={{fontSize: 12, fontWeight: 'bold'}} />
                <YAxis allowDecimals={false} />
                <Tooltip cursor={{fill: '#f8fafc'}} />
                <Bar dataKey="Reportes" fill="#f59e0b" radius={[8, 8, 0, 0]} barSize={60} label={{ position: 'top', fill: '#92400e', fontWeight: 'bold' }} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
    ...
    () => (
      <div className="flex flex-col items-center justify-center h-full p-12 text-center bg-indigo-900 rounded-[3rem] mx-8 my-8 shadow-2xl shadow-indigo-500/20 text-white relative overflow-hidden">
        ...
        <h2 className="text-5xl font-black mb-6 relative z-10">Conclusiones y Acuerdos</h2>
        <p className="text-xl text-indigo-200 max-w-2xl mx-auto mb-12 relative z-10 leading-relaxed">
          Apertura de la mesa de debate directivo. Basados en la data presentada, definir el plan de acción táctico para el próximo mes.
        </p>
        ...
        <div className="w-full max-w-3xl bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-6 text-left relative z-10">
          <ul className="list-disc list-inside space-y-3 text-lg font-medium text-indigo-50">
            <li>¿Qué infracción requiere nueva normativa o campaña de concientización?</li>
            <li>Revisión de estudiantes recurrentes con el departamento de Psicología.</li>
            <li>Reevaluación de zonas de patrullaje basándonos en los grados más conflictivos.</li>
          </ul>
        </div>
      </div>
    )
  ...
  if (isLoading || !stats) {
    return (
      <div className="h-full flex flex-col items-center justify-center gap-4 bg-slate-50 min-h-[600px] rounded-3xl border border-slate-200">
        ...
        <p className="text-slate-500 font-bold animate-pulse">Agrupando análisis de Big Data...</p>
      </div>
    );
  }

  return (
    <div className={`transition-all duration-300 ${isFullscreen ? 'fixed inset-0 z-[100] bg-slate-900 p-4' : 'h-[calc(100vh-100px)] min-h-[700px] bg-slate-100 rounded-3xl border border-slate-200 shadow-inner p-4 relative overflow-hidden flex flex-col'}`}>
      ...
      <div className="shrink-0 mt-4 flex items-center justify-between bg-white px-6 py-4 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex items-center gap-4">
          <button onClick={toggleFullscreen} className="p-3 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-colors tooltip-trigger" title="Pantalla Completa (F)">
