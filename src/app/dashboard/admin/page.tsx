...
export default function AdminPage() {
  ...
  const resetForm = () => {
    setFormData({ name: '', dni: '', role: 'BRIGADIER_AULA', password: '', grade: '', section: '' });
    setIsEditing(false);
    setEditingId(null);
  };
  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!payload.role.includes('BRIGADIER_AULA') && !payload.role.includes('BRIGADIER_PATRULLA')) {
         payload.grade = null;
         ...
         <div className='bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden animate-in fade-in duration-300'>
            <div className='p-4 border-b border-slate-100 flex flex-wrap justify-between items-center gap-4 bg-slate-50/50'>
               ...
               <div className='flex gap-2'>
                  <Button onClick={handleOpenCreate} size='sm' className='bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-500/20'>
                           ...
                           <td className='px-6 py-3'>
                              <div className='flex justify-end gap-1'>
                                {user.role?.includes('BRIGADIER') && (
                                    <button onClick={() => handlePreviewCarnet(user)} className='text-slate-400 hover:text-indigo-600 transition-colors p-2' title="Ver Carnet">
                                        <Eye className='w-4 h-4' />
                                    </button>
                                )}
                                <button onClick={() => handleOpenEdit(user)} className='text-slate-400 hover:text-blue-600 transition-colors p-2' title="Editar">
                                    <Edit className='w-4 h-4' />
                                </button>
                                <button onClick={() => handleDeleteUser(user.id)} className='text-slate-400 hover:text-red-600 transition-colors p-2' title="Eliminar">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
             <div className="flex justify-between items-center p-4 border-b border-slate-100">
              <h3 className="font-bold text-lg text-slate-800">{isEditing ? 'Editar Usuario' : 'Crear Nuevo Usuario'}</h3>
            ...
            <form onSubmit={handleSaveUser} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
               ...
               <div className="pt-4 flex justify-end gap-3 sticky bottom-0 bg-white">
                  ...
                  <Button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-md shadow-indigo-500/20">
