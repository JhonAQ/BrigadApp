...
import { Shield, Plus, Edit2, Key, CheckCircle2, UserCog, User, LayoutGrid, Trash2, Printer, CheckSquare } from 'lucide-react';
...
export default function AdminPage() {
    ...
    const [activeTab, setActiveTab] = useState<'admins' | 'brigadiers' | 'credentials'>('admins');
    ...
    const openCreateModal = () => {
        ...
        setFormData({ dni: '', password: generatedPassword, name: '', role: 'BRIGADIER_AULA', grade: '', section: '' });
        setIsEditing(false);
        setEditingId(null);
        setIsModalOpen(true);
    };
    ...
    const handleDeleteUser = async (userToDelete: any) => {
        if (!confirm(`¿Estás seguro que deseas ELIMINAR permanentemente a ${userToDelete.name} (${userToDelete.id})?`)) return;
    ...
    const handleSaveUser = async () => {
        if (!formData.dni || !formData.name || !formData.role) return alert('Completa los campos base');

        if (requiresClassroom && (!formData.grade || !formData.section)) {
            return alert('Si el rol es Brigadier de Aula o Patrulla, debes asignar grado y sección obligatoriamente.');
        }
        ...
        if (isEditing && editingId) {
            ...
            if (error) return alert('Error creando usuario. Quizá el DNI ya existe: ' + error.message);
        }
    ...
    const requiresClassroom = formData.role === 'BRIGADIER_AULA' || formData.role === 'BRIGADIER_PATRULLA';

    const admins = users.filter(u => u.role === 'DEVELOPER' || u.role === 'DOCENTE' || u.role?.includes('BRIGADIER_GENERAL') || u.role === 'PSYCHOLOGIST');
    const brigadiersAula = users.filter(u => u.role === 'BRIGADIER_AULA');
    const brigadiersPatrulla = users.filter(u => u.role === 'BRIGADIER_PATRULLA');
    const allBrigadiers = [...brigadiersAula, ...brigadiersPatrulla, ...users.filter(u => u.role?.includes('GENERAL'))];

    if (user?.role !== 'DEVELOPER' && user?.role !== 'DOCENTE' && !user?.role?.includes('BRIGADIER_GENERAL')) {
        return (
            <div className='flex items-center justify-center p-10'>
                <p className='text-red-500 font-bold'>No tienes permisos para ver esta página.</p>
            </div>
        );
    }

    const usersList = (list: any[], mode: 'list' | 'select' = 'list') => {
        if (list.length === 0) return <div className='py-8 text-center text-slate-400'>No hay usuarios aquí.</div>;
                    ...
                    <table className='w-full text-left border-collapse'>
                        <thead>
                            <tr className='bg-slate-50 border-b border-slate-200 text-xs uppercase text-slate-500 font-semibold tracking-wider'>
                                ...
                                {mode === 'list' && <th className='p-4 whitespace-nowrap text-right'>Acciones</th>}
                            </tr>
                        </thead>
                        <tbody className='divide-y divide-slate-100'>
                            {list.map((u) => (
                                <tr key={u.id} className={`hover:bg-slate-50/50 transition-colors ${selectedForPrint.includes(u.id) ? 'bg-indigo-50/30' : ''}`}>
                                    {mode === 'select' && (
                                    ...
                                    <td className='p-4'>
                                        <div className='flex items-center gap-3'>
                                            ...
                                            <div>
                                                <p className='font-bold text-slate-900 text-sm whitespace-nowrap'>{u.name}</p>
                                                <p className='text-xs text-slate-500'>Doc/Usuario: {u.dni || u.id}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className='p-4'>
                                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-bold rounded-full border whitespace-nowrap ${
                                            u.role === 'DOCENTE' || u.role === 'PSYCHOLOGIST' ? 'bg-blue-50 text-blue-700 border-blue-100' :
                                            u.role.includes('GENERAL') ? 'bg-purple-50 text-purple-700 border-purple-100' :
                                            'bg-emerald-50 text-emerald-700 border-emerald-100'
                                        }`}>
                                            {u.role === 'DOCENTE' ? <Shield className='w-3 h-3' /> : <User className='w-3 h-3' /> }
                                            {u.role.replace(/_/g, ' ')}
                                        </span>
                                    </td>
                                    <td className='p-4'>
            ...
            <div className='flex gap-2 p-1 bg-slate-100 border border-slate-200 rounded-xl overflow-x-auto whitespace-nowrap max-w-fit'>
                <button
                    ...
                    className={`px-5 py-2 text-sm font-semibold rounded-lg transition-all ${
                        activeTab === 'admins' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                    }`}
                >
                    Administrativos
                </button>
                <button
                    ...
                    className={`px-5 py-2 text-sm font-semibold rounded-lg transition-all ${
                        activeTab === 'brigadiers' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                    }`}
                >
                ...
                <button
                    ...
                    className={`px-5 py-2 text-sm font-semibold rounded-lg transition-all flex items-center gap-2 ${
                        activeTab === 'credentials' ? 'bg-white text-blue-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                    }`}
                >
                        ...
                        <div className='space-y-8'>
                            <div className='space-y-4'>
                                <div className='flex items-center gap-2 border-b border-slate-100 pb-2 mb-2'>
                                    ...
                                    <h3 className='font-bold text-slate-800 text-lg'>Brigadieres de Aula</h3>
                                    <span className='bg-slate-100 text-slate-500 text-xs font-bold px-2 py-0.5 rounded-md'>{brigadiersAula.length}</span>
                                </div>
                            ...
                            <div className='space-y-4'>
                                <div className='flex items-center gap-2 border-b border-slate-100 pb-2 mb-2'>
                                    ...
                                    <h3 className='font-bold text-slate-800 text-lg'>Brigadieres de Patrulla</h3>
                                    <span className='bg-slate-100 text-slate-500 text-xs font-bold px-2 py-0.5 rounded-md'>{brigadiersPatrulla.length}</span>
                                </div>
                    ...
                    {activeTab === 'credentials' && (
                        <div className='space-y-6'>
                            <div className='flex items-center justify-between border-b border-slate-200 pb-4'>
                                <div>
                                    <h3 className='text-lg font-bold text-slate-900'>Generador de Identificaciones</h3>
                                    <p className='text-sm text-slate-500'>Selecciona los brigadieres que necesitan carnet impreso.</p>
                                </div>
                                <div className="flex gap-3">
                                    <button 
                                        onClick={() => setSelectedForPrint(allBrigadiers.map(b => b.id))}
                                        className="text-sm font-semibold text-slate-600 border border-slate-200 bg-white px-4 py-2 rounded-lg"
                                    >
                                        Marcar Todos
                                    </button>
                                    <Link href={`/print/credentials?ids=${selectedForPrint.join(',')}`}>
                                        <button 
                                        ...
                                        >
                                            <Printer className='w-4 h-4' /> Imprimir {selectedForPrint.length} Carnets PDF
                                        </button>
                                    </Link>
                                </div>
                            </div>
                ...
                <div className='fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4'>
                    <div className='bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-200'>
                        <div className='p-5 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between'>
                            ...
                            <button onClick={() => setIsModalOpen(false)} className='text-slate-400 hover:text-slate-600 bg-slate-200/50 hover:bg-slate-200 p-1.5 rounded-full transition-colors'>
                        ...
                        <div className='p-6 space-y-4'>
                            <div className='space-y-1.5'>
                                ...
                                />
                                {isEditing && <p className='text-[10px] text-slate-400'>El ID de acceso no se puede modificar.</p>}
                            </div>
                            ...
                            <div className='space-y-1.5'>
                                ...
                                />
                                <p className='text-[10px] text-slate-500'>Por defecto se genera automática de 6 dígitos al crear.</p>
                            </div>
                            ...
                            <div className='space-y-1.5'>
                                ...
                                <select
                                    ...
                                    value={formData.role}
                                    onChange={(e) => setFormData({ ...formData, role: e.target.value, grade: '', section: '' })}
                                >
                            ...
                            {requiresClassroom && (
                                <div className='grid grid-cols-2 gap-4 pt-2 border-t border-slate-100'>
                                    ...
                                    <div className='space-y-1.5'>
                                        <label className='text-xs font-semibold text-slate-700'>Sección</label>
                                        <select
                        ...
                        <div className='p-5 border-t border-slate-100 bg-slate-50 flex gap-3 justify-end'>
                            ...
                            >
                                <CheckCircle2 className='w-4 h-4' /> {isEditing ? 'Guardar Cambios' : 'Registrar'}
                            </button>
                        </div>
                    </div>
                </div>
