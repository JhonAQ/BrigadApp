       ...
       <>
        <div className='flex items-center justify-center mb-6'>
             <div className='flex items-center gap-2'>
                 {[1, 2, 3].map((s) => (
                     <div key={s} className={`w-2.5 h-2.5 rounded-full transition-colors ${step >= s ? 'bg-indigo-600' : 'bg-slate-200'}`} />
                 ))}
             </div>
        </div>
        ...
        {step === 1 && (
            ...
            <h2 className='text-lg font-bold mb-4 text-slate-800'>1. Identificar Alumno</h2>
            
            <div className='grid grid-cols-2 gap-4 mb-4'>
                <div>
                   <label className='text-xs font-bold text-slate-500 uppercase mb-1 block'>Grado</label>
                   <select 
                ...
                <div>
                   <label className='text-xs font-bold text-slate-500 uppercase mb-1 block'>Sección</label>
                   <select 
                     className='w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:border-indigo-500 transition-colors'
            ...
            <div className='mt-6 flex justify-end'>
                <Button onClick={handleNext} disabled={!selectedStudent} className='bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-500/20'>
                Siguiente
        ...
        {step === 2 && (
            <div className='bg-white p-6 rounded-2xl shadow-sm border border-slate-100 animate-in fade-in slide-in-from-right-4 duration-300'>
            <h2 className='text-lg font-bold mb-4 text-slate-800'>2. Detalles del Evento</h2>
            
            <label className='text-xs font-bold text-slate-500 uppercase mb-2 block'>Nivel de Gravedad</label>
            <div className='grid grid-cols-3 gap-3 mb-6'>
                {[
                    { id: 'LEVE', label: 'Leve', color: 'bg-emerald-50 text-emerald-700 border-emerald-200 ring-emerald-200', icon: ShieldAlert },
                    { id: 'MODERADA', label: 'Moderada', color: 'bg-amber-50 text-amber-700 border-amber-200 ring-amber-200', icon: AlertTriangle },
                    { id: 'GRAVE', label: 'Grave', color: 'bg-rose-50 text-rose-700 border-rose-200 ring-rose-200', icon: FileWarning },
                ].map((opt) => (
                    <button
                        key={opt.id}
                        ...
                        `}
                    >
                        <opt.icon className={`w-6 h-6 mb-1 ${severity === opt.id ? 'scale-110' : ''}`} />
        ...
        {step === 3 && (
            <div className='bg-white p-6 rounded-2xl shadow-sm border border-slate-100 animate-in fade-in slide-in-from-right-4 duration-300'>
            <h2 className='text-lg font-bold mb-4 text-slate-800'>3. Confirmar Reporte</h2>
            ...
            <div className='bg-slate-50 rounded-xl p-5 space-y-4 mb-6 border border-slate-100'>
                ...
                <div className='grid grid-cols-2 gap-4'>
                    <div>
                         <p className='text-xs text-slate-400 uppercase font-bold mb-1'>Gravedad</p>
                    ...
                    <div>
                        <p className='text-xs text-slate-400 uppercase font-bold mb-1'>Hora</p>
                        <p className='text-sm font-medium text-slate-700'>{new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
                    </div>
                </div>
                
                <div>
                   <p className='text-xs text-slate-400 uppercase font-bold mb-1'>Observación</p>
                   <p className='text-sm text-slate-600 leading-relaxed bg-white p-3 rounded-lg border border-slate-200'>{description}</p>
                </div>
            </div>
